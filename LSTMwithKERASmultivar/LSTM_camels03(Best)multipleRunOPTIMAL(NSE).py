...
#print("values shape before scale: ",values.shape)
NNtype = 'GRU'
div = 5
batch_size = 50
epochs = 100
...
titletxt = "{} Nash-Sutcliffe coefficient (NSE)".format(NNtype)
...
def NeuralNetwork():
	...
	values1 = tempReframed.values
	n_train_days = 15 * 365
	train = values1[:n_train_days, :]
	test = values1[n_train_days:, :]
	...
	nse = calc_nse(inv_y,inv_yhat)
	return [rmse,nse]
for i in range(num_runs):
	output = NeuralNetwork()
	rmse = output[0]
	nse = output[1]
	print("rmse: {} for run {}".format(rmse,i))
	print("NSE: {} for run {}".format(nse,i))
	nseList.append(nse)
fig = pyplot.figure(figsize=(6,5))
axes = fig.add_axes([0.2,0.2,0.7,0.7])
axes.plot(runsList,nseList,color='tab:blue',marker='o',linestyle='None')
pyplot.ylim((0,1))
...
pyplot.savefig("MultipleRunsLoss3.png")
