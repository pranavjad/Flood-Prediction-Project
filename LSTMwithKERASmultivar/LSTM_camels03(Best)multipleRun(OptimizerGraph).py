...
loss = 'mae'
optimizers = ['adam','sgd','rmsprop','adagrad','adadelta','adam','adamax','nadam']
num_runs = 5
...
txt = 'batch_size: {}, epochs: {}, input neurons: {}, output activation: {}, loss: {}, number of runs: {}'.format(
    batch_size,epochs, input_neurons, output_activation, loss, num_runs)
...
for i in optimizers:
	...
	axes.plot(runsList, rmseList, color=color,marker='o',label=str(optimizer),linestyle='None')
	colorIndex+=1
pyplot.figtext(0.5,0.03,txt,wrap=True,horizontalalignment='center',fontsize=12)
axes.legend(loc='upper left',bbox_to_anchor=(1,1),fontsize='xx-small',title='Optimizer')
pyplot.savefig("MultipleRunsRMSE(OptimizerGraph)2.png")
