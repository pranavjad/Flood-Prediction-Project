...
losses = ['mean_absolute_error','mean_squared_error','mean_absolute_percentage_error','mean_squared_logarithmic_error','squared_hinge','hinge','categorical_hinge','logcosh','huber_loss','binary_crossentropy','kullback_leibler_divergence']
optimizer = 'adam'
num_runs = 5
...
txt = 'epochs: {}, input neurons: {}, output activation: {}, optimizer: {}, number of runs: {}'.format(
    batch_size, input_neurons, output_activation, optimizer, num_runs)
fig = pyplot.figure(figsize=(7,5))
axes = fig.add_axes([0.1,0.2,0.6,0.7])
pyplot.ylim((200,500))
pyplot.xticks(runsList)
pyplot.xlabel("Run Number")
pyplot.ylabel("RMSE")
fig.suptitle(titletxt,fontsize=16)
colorList = ['tab:blue','tab:orange','tab:green','tab:red','tab:purple','tab:brown','tab:pink','tab:gray','tab:olive','tab:cyan','black','lime','navy']
colorIndex = 0
for i in losses:
	loss = i
	rmseList = []
	...
	for i in range(num_runs):
		...
		print(rmse)
		print("CURRENTLY ON",loss)
		rmseList.append(rmse)
		del values,scaled,reframed,tempReframed,test_X,test_y,train_X,train_y,tempTest_X,inv_yhat,inv_y
	axes.plot(runsList, rmseList, color=color,marker='o',label=str(loss),linestyle='None')
	colorIndex+=1
pyplot.figtext(0.5,0.03,txt,wrap=True,horizontalalignment='center',fontsize=12)
axes.legend(loc='upper left',bbox_to_anchor=(1,1),fontsize='xx-small',title='Losses')
pyplot.savefig("MultipleRunsRMSE(LossesGraph)3.png")
